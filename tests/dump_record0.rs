...
fn main() -> std::io::Result<()> {
    ...
    let rec1_offset = u32::from_be_bytes([
        data[78 + 8],
        data[78 + 9],
        data[78 + 8 + 2],
        data[78 + 8 + 3],
    ...
    println!(
        "Record 0 offset: 0x{:04X} ({} bytes)",
        mobi_offset, mobi_offset
    );
    println!(
        "Record 1 offset: 0x{:04X} ({} bytes)",
        rec1_offset, rec1_offset
    );
    ...
    println!(
        "Compression: {}",
        u16::from_be_bytes([rec0_data[0], rec0_data[1]])
    );
    ...
    let header_len =
        u32::from_be_bytes([rec0_data[20], rec0_data[21], rec0_data[22], rec0_data[23]]);
                    ...
                    if pos + 8 <= rec0_data.len() {
                        ...
                        if pos + 8 + (rec_len as usize) <= rec0_data.len() {
                            ...
                            println!(
                                ...
                                rec_type,
                                rec_len,
                                String::from_utf8_lossy(rec_data)
                                    .chars()
                                    .take(50)
                                    .collect::<String>()
                            );
