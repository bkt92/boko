...
fn main() -> io::Result<()> {
    ...
    println!("Reference magic: {:?}", String::from_utf8_lossy(&ref_data[60..68]));
    println!("Generated magic: {:?}", String::from_utf8_lossy(&gen_data[60..68]));
    ...
    for i in 0..5 {
        ...
        println!("Record {}: ref={}, gen={}, diff={}", i, ref_off, gen_off, gen_off as i64 - ref_off as i64);
    }
    ...
    println!("\nCompression type:");
    println!("  Reference: {}", u16::from_be_bytes([ref_data[ref_mobi_off], ref_data[ref_mobi_off + 1]]));
    println!("  Generated: {}", u16::from_be_bytes([gen_data[gen_mobi_off], gen_data[gen_mobi_off + 1]]));

    println!("\nText length:");
    println!("  Reference: {}", u32::from_be_bytes([
        ref_data[ref_mobi_off + 4],
        ref_data[ref_mobi_off + 5],
        ref_data[ref_mobi_off + 6],
        ref_data[ref_mobi_off + 7]
    ]));
    println!("  Generated: {}", u32::from_be_bytes([
    ...
    println!("\nText record count:");
    println!("  Reference: {}", u16::from_be_bytes([ref_data[ref_mobi_off + 8], ref_data[ref_mobi_off + 9]]));
    println!("  Generated: {}", u16::from_be_bytes([gen_data[gen_mobi_off + 8], gen_data[gen_mobi_off + 9]]));

    println!("\nText record size:");
    println!("  Reference: {}", u16::from_be_bytes([ref_data[ref_mobi_off + 10], ref_data[ref_mobi_off + 11]]));
    println!("  Generated: {}", u16::from_be_bytes([gen_data[gen_mobi_off + 10], gen_data[gen_mobi_off + 11]]));
    ...
    println!("Reference images: {} (first at record {})", ref_image_offsets.len(), ref_image_offsets.first().unwrap_or(&0));
    println!("Generated images: {} (first at record {})", gen_image_offsets.len(), gen_image_offsets.first().unwrap_or(&0));
    ...
    if !ref_image_offsets.is_empty() && !gen_image_offsets.is_empty() {
        ...
        let ref_img_data = &ref_data[ref_img_start..ref_img_start + 20.min(ref_data.len() - ref_img_start)];
        let gen_img_data = &gen_data[gen_img_start..gen_img_start + 20.min(gen_data.len() - gen_img_start)];
